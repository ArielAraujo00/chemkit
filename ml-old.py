...
def umap_hdbscan(data, return_models=False,
    ...
    embedding = pd.DataFrame(reducer.fit_transform(data), index=data.index, columns=[f'UMAP{col+1}' for col in range(n_components)])
...
def multi_objective_eval(embedding, labels, data=None, objectives=('ss',), sample_size=2000, n_neighbors=15, klow=8, khigh=16, kpwr=2):
    ...
    if any(m for m in manifold_metrics if m in objectives) and data is None:
        raise ValueError("'data' must be provided for any manifold metric.")
    ...
    if 'global' in objectives:
        if len(data) > sample_size:
            ...
            emb_sub = embedding.iloc[idx]
        else:
            data_sub = data
            emb_sub = embedding
        scores['global_structure'] = spearmanr(pdist(data_sub), pdist(emb_sub)).correlation
    ...
    if 'knn' in objectives:
        ...
        for i in range(len(data)):
            intersection = len(set(high_idx[i]).intersection(low_idx[i])) / n_neighbors
            overlap.append(intersection)
        scores['knn_preservation'] = np.mean(overlap)
